...
def bayes_analysis(base_suc_rate, control_group, variant_group, control_successes, variant_successes, lift_percent_desired):
    ...
    variant_up_test = btdtri(alfa_test, beta_test, 0.975)
    variant_low_test = btdtri(alfa_test, beta_test, 0.25)

    control_up_control = btdtri(alfa_control, beta_control, 0.975)
    control_low_control = btdtri(alfa_control, beta_control, 0.25)
    ...
    return dict(
        ...
        conf_interval = ((round(control_low_control*100,2),(round(control_up_control*100,2))),(round(variant_low_test*100,2),round(variant_up_test*100,2))),
...
def frequentist_analysis(control_group, control_successes, variant_group, variant_successes, test_type):
    ...
    pooled_se = np.sqrt((control_rate * (1 - control_rate) / control_group) + (variant_rate * (1 - variant_rate) / variant_group))
